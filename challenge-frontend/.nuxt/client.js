...
function mapTransitions(toComponents, to, from) {
  ...
  for (let i = 0; i < maxDepth; i++) {
    ...
    Object.keys(toTransitions)
      .filter(
        (key) => typeof toTransitions[key] !== 'undefined' && !key.toLowerCase().includes('leave'),
      )
      .forEach((key) => {
...
async function loadAsyncComponents(to, from, next) {
  ...
  try {
    if (this._queryChanged) {
      const Components = await resolveRouteComponents(to, (Component, instance) => ({
        Component,
        instance,
      }))
  ...
  try {
    ...
    await Promise.all(
      Components.map(async (Component, i) => {
        ...
        const hasAsyncData =
          Component.options.asyncData && typeof Component.options.asyncData === 'function'
